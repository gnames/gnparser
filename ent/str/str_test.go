...
func TestStringTools(t *testing.T) {
	t.Run("CapitalizeName", func(t *testing.T) {
		...
		for _, v := range tests {
			...
			assert.Equal(t, v.out, res, v.msg)
		}
	})

	t.Run("NormalizePreservingDiaereses", func(t *testing.T) {
		tests := []struct {
			msg string
			in  string
			out string
		}{
			{"Döringina", "Döringina", "Doeringina"},
			{"Lecythis paraënsis", "Lecythis paraënsis", "Lecythis paraënsis"},
			{"thomæ", "thomæ", "thomae"},
			{"many ö", "ööö", "oeoeoe"},
			{"’", "’", ""},
			{"‘", "‘", ""},
		...
		for _, v := range tests {
			res := str.NormalizePreservingDiaereses(v.in)
			assert.Equal(t, v.out, res, v.msg)
		}
	})
